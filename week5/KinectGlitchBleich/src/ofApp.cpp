...
ofVec3f getVertexFromImg(ofImage& img, int x, int y) {
	...
	if(cur.getBrightness() > 180 ) {
		float z = ofMap(cur.getBrightness(), 200, 255, -480, 480);
...
void ofApp::update() {
	...
	if(kinect.isFrameNew()) {
        ...
        glitchKinect.saveImage("out1.jpeg",OF_IMAGE_QUALITY_HIGH);
	...
	//	} 
        ...
        ofVec3f zero;
        zero.set(0,0,0);
        for(int y = 0; y < glitchKinect.getHeight()-4; y+=4) {
            for(int x = 0; x < glitchKinect.getWidth()-4; x+=4) {
                ...
                ofVec3f ne = getVertexFromImg(glitchKinect, x + 4, y);
                ofVec3f sw = getVertexFromImg(glitchKinect, x, y + 4);
                ofVec3f se = getVertexFromImg(glitchKinect, x + 4, y + 4);
                ...
                if(nw != zero && ne != zero && sw != zero && se != zero && theDistance<.03) {
